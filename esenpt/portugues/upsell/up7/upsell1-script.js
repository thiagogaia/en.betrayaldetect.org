(function(){
    ...
    // COUNTDOWN TIMER
    // ============================================
    var STORAGE_KEY = 'upsell7_timer_end';
    ...
    // ============================================
    var firstNames = [
        'Ana', 'João', 'Maria', 'Pedro', 'Camila', 'Carlos', 'Juliana', 'Rafael',
        'Sofia', 'Bruno', 'Isabella', 'Lucas', 'Fernanda', 'Daniel', 'Mariana',
        'Gabriel', 'Beatriz', 'Matheus', 'Larissa', 'André', 'Jéssica', 'Rodrigo',
        'Patrícia', 'Felipe', 'Amanda', 'Thiago', 'Raquel', 'Luís', 'Nicole',
        'Aline', 'Breno', 'Cristina', 'Diego', 'Elena', 'Fábio', 'Graça'
    ];
    
    var locations = [
        'São Paulo', 'Rio de Janeiro', 'Belo Horizonte', 'Salvador', 'Brasília', 'Curitiba',
        'Fortaleza', 'Recife', 'Porto Alegre', 'Manaus', 'Campinas', 'Goiânia',
        'Belém', 'Guarulhos', 'São Luís', 'Florianópolis', 'Vitória', 'Santos',
        'Natal', 'São Bernardo', 'Niterói', 'Joinville', 'Uberlândia', 'Maceió'
    ];
    
    var actions = [
        'ativou Análise de Comportamento por IA',
        'desbloqueou detecção de padrões',
        'habilitou rastreamento de comportamento por IA',
        'fez upgrade para Analista IA'
    ];
    ...
    function getRandomTime() {
        ...
        if (rand < 0.3) return Math.floor(Math.random() * 60) + ' segundos atrás';
        if (rand < 0.7) return (Math.floor(Math.random() * 5) + 1) + ' minutos atrás';
        return (Math.floor(Math.random() * 10) + 5) + ' minutos atrás';
    }
