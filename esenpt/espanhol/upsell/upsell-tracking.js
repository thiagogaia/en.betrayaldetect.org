 ...
 * visitorId can be passed via URL (vid/visitorId) to maintain tracking continuity
 */
...
const UpsellTracker = {
    ...
    getVisitorId: function() {
        // Priority 1: Check URL parameter (cross-domain tracking)
    ...
    setupListeners: function() {
        ...
        // Track buy button clicks (PerfectPay / centerpag or CTA button)
        document.querySelectorAll('a[href*="centerpag.com"], .btn-primary.btn-mega, [data-checkout-link]').forEach(btn => {
        ...
        document.querySelectorAll('.decline-link, a[href*="up2"], a[href*="up3"], a[href*="up4"], a[href*="up5"], a[href*="up6"], a[href*="up7"], a[href*="thankyou"]').forEach(link => {
            ...
            if (!link.classList.contains('btn-mega') && !link.hasAttribute('data-checkout-link') && !link.href.includes('centerpag.com')) {
    ...
    setupEngagementTracking: function() {
        ...
        window.addEventListener('scroll', function() {
            ...
            if (!trackedCTA) {
                const ctaButton = document.querySelector('.btn-primary.btn-mega, a[href*="centerpag.com"], [data-checkout-link]');
                if (ctaButton) {
