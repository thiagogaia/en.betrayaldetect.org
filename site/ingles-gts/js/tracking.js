...
const WhatSpyTracking = {
    ...
    trackButtonClicks: function() {
        // Skip generic ButtonClick on quiz - we use dedicated quiz events instead
        if (window.location.pathname.indexOf('/quiz/') !== -1) return;
        document.addEventListener('click', (e) => {
            const button = e.target.closest('button, .btn, [role="button"]');
            if (!button) return;
            // Only track buttons explicitly marked for tracking, or primary CTAs
            const isCta = button.matches('[data-track], .btn-primary, .cta-button, .cta-primary, .cta-final, .checkout-btn, [data-cta]');
            if (!isCta) return;
     ...
     */
    trackAddToCart: function(value = 47, currency = 'USD') {
